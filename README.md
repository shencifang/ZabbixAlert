...
- **告警去重**：以 lastchange（告警时间戳）为唯一标识，已存在且未关闭的告警自动忽略
...
- **版本号更新**：从 v0.5 → 0.6，窗口标题改为 `ZABBIX告警监测（0.6）`
- **告警去重键变更**：去重唯一标识从 `triggerid` 改为 `lastchange`（告警时间戳），避免同一触发器重复关闭后无法再次告警
- **数据持久化键同步**：事件入库时 `eventModule.id` 从 `triggerid` 改为 `lastchange`，保持与去重逻辑一致
- **关闭事件 API 路径复原**：关闭链接路径改回 `/api.php`（v0.5 曾调整为 `/`）
- **界面字体调整**：`label6`、`label7` 显示字体从"黑体"改为"Microsoft Sans Serif"

### v0.5

- **版本号更新**：从 beta0.4 → 0.5，窗口标题改为 `ZABBIX告警监测（0.5）`
- **Zabbix 登录全局化**：主轮询定时器（timer1_Tick）使用类成员变量 `zabbix`，程序启动时登录并保持长连接，不再每次取数重新登录（注：手动取数按钮仍局部创建新连接）
- **告警去重**：以 triggerid 为唯一标识，已存在未关闭的告警自动忽略，避免重复通知
- **恢复企业微信通知**：`send()` 调用从注释状态恢复，新告警自动推送
- **告警消息含关闭链接**：推送消息包含可点击的关闭事件链接（路径为 `/api.php`）
